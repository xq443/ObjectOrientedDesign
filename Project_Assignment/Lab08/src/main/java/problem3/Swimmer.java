...
public class Swimmer implements Comparable<Swimmer> {
  ...
  public int compareTo(Swimmer otherSwimmer) {
    // Compare based on the minimum butterfly 50m time
    double thisMinTime = Collections.min(this.butterfly50mTimes);
    double otherMinTime = Collections.min(otherSwimmer.getButterfly50mTimes());
    return Double.compare(thisMinTime, otherMinTime);
  }
